...
| 站点元素 | 对应 TUI 的实现 |
|---|---|
| 星场（星芒 + 流星）/ 点阵 / 斜纹 | `/background`（`appearance.rs` 的 `Background`） |
...
- **背景纹理：stars 用 canvas，规律纹理用 CSS**。stars 要点光叠加与动画，CSS 表达不了；
  dots / diagonal 是规律图案，用 CSS 渐变的覆盖率精确可控（canvas 画会被 DPR 缩放
  弄糊、且覆盖率难估）。**改完必须量像素**：分别与 `bg=none` 做逐像素比对，
  确认"明显不同像素占比"在 2%~8% —— 这是 TUI 注释里标定的"看得见但不抢眼"区间
  （超过 14% 会盖过正文）。曾经因为一层 `opacity:0.92` 的遮罩，canvas 上画了
  588 个像素却屏幕上完全看不见。
- **星场的动画与 TUI 的关系**：TUI 的星场是**静止**的（确定性 `cell()`，因为终端
  每帧全量重画、随机纹理闪成噪点）。官网要有星芒与流星就必须动。折中做法：
  星点**位置/大小用固定种子**（每次加载一致，不噪），只有**亮度**随时间呼吸。
  动画验收要连拍多帧看帧间差异，且流星是随机事件 —— 需采样 ≥15 秒或直接
  按像素尖峰检测（我第一版把拖尾长度当"速度比例"除了，尾巴只有 1~4px，
  肉眼完全看不到流星，靠像素检测才发现）。
