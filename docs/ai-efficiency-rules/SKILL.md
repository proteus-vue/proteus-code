...
**前提**：用户可能正在同一台机器上做别的工作。GUI 验证、截图、启动应用这类动作
**默认会抢走前台焦点**，等于强行打断用户 —— 这比"慢"更严重。

| 禁止 | 正确做法 |
|---|---|
| `open -a <App>`、`activate=true` 把窗口抢到前台 | **按窗口 ID 截图**：`screencapture -x -l <windowid> out.png`（不激活、不抢焦点） |
| 为了"看一眼"反复激活同一个窗口 | 一次取到图后**离线读文件**（Read 图片），不要反复激活 |
| 让用户"等我测完" | 需要交互的验证要么脚本化（启动+操作+采集+退出一体），要么先告知并让用户挑时机 |

**判据**：若一条验证路径必须让用户停下手上的事，它就已经失败了 ——
先改验证手段，再谈验证。本项目约定：**GUI 截图用窗口 ID，不用激活**。
（与"效率"并列，属正确性与尊重范畴，非可选项。）
