...
| # | 项 | 对标目标 | 我们 | 数据在事件流里吗 |
|---|---|---|---|---|
| D1 | 左侧任务/工作区栏（状态、`+/-` 行数、分组） | ZCode | ⚠️ **已做会话栏**（列表 + 当前项高亮 + 新建 + 可切回；当前会话即使尚未落盘也会显示）。**未做**：按工作区分组、运行/未读/失败状态圆点、`+/-` 行数、Grouped/Workspace/Timeline 视图、Archive、搜索 —— 这些需要会话元数据里有工作区与运行状态，当前 `SessionStore` 只存 id/标题/记录数（**列出来但显示不出来是比不列更糟的假象**，故不做） | 部分 |
