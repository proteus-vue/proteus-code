...
> **2026-09-21 更新（此前这句话只对一半）**：原文写的是"我们的 `Op`/`EventMsg` 已经是这个角色，
> 且更强"。**角色**确实相同，但当时那份只是**同进程 Rust 类型**，对外真正可用的只有 Web 宿主的
> 6 条路由（`Op` 的 17 个变体线上只映射了 4 个）—— 没有 schema/版本/握手，也没有跨进程传输。
> 现在 `neo-host-appserver` 补上了那一半：stdio 上的 JSON-RPC 2.0、**17 个方法覆盖全部 `Op`**、
> `initialize` 握手报 `SCHEMA_VERSION` 与方法表、事件以带 `seq` 的通知推送、审批用
> `approval/respond` 应答内核给的同一个 id。**"更强"那半仍然成立**（golden 回放与 T6 等价断言
> 是 Codex 那套没有的确定性纪律）；**还没做**的是 `ts-rs` 那半 —— 类型导出的产物，
> 见 `PROJECT_MEMORY.md` 接续点的"下次可接着做③"。
...
| D5 | **diff 渲染** | 两者 | ✅ **已实现**（按行着色；顺带修掉"预览与执行可能不是同一文件"的缺陷）<br>**已补行背景带**（gpui 宿主）：新增/删除/hunk 头各一层底，让"改动落在哪几段"一眼扫得出 —— 此前只有逐行彩色文字，给不出那个形状。走渲染缝自绘（`diff_backdrop`），对齐由"底带与文字共用同一行高"构造保证，真机实测每行 26pt 严丝合缝（见 §4.64(ay)）。<br>**折叠未改区块：已生效** —— `neo-ui-behavior::fold` 只折连续 >6 行的上下文，而审批预览的上下文半径已由 3 提到 **10**（用户拍板），超过折叠阈值 6，故**已真正生效**：真机实测 25 行 diff 折成 14 行，`⋯ 未改 4 行`/`7 行` 与算法逐项对得上。两者耦合有断言守护（见 §4.64(bc)）。<br>**已补行内字符级高亮**：真正变化的字压一层更浓的底（用底色而非粗体 —— CJK 无真粗体字面，加粗等于没做；真机实测 29→89 差 60 色阶）。判定在共享层 `inline_emphasis`，两宿主共用。<br>**并排视图与跳转改动已做**（gpui 宿主，全屏查看器）：`/diff` 或审批时按 `d` 打开，`v` 切统一/双列、`]` `[` 跳上/下一处改动、`n` `p` 跳文件、`j` `k` `g` `G` 移动、`b` 开关文件树。**键位与 TUI 的全屏查看器逐键一致** —— 两个宿主对同一份 diff 提供同一套操作，用户不必学两遍。<br>落地时做了一次**上移**（而不是抄一份）：解析 + hunk/文件导航 + 并排配对原本在 `neo-host-tui::diffview`，现移到 `neo-capability::diff_view`（与生产者 `unified_diff` 同处一地），TUI 改成再导出。理由具体：抄一份的话，**同一个 `@@` 头会在两个宿主里被解析出不同的 hunk**（TUI 说"下一处改动在这"，GUI 跳去别处），而解析器离生产者越远、格式一改就越容易只有一侧跟上。<br>并排的**配对规则**也在共享层（`Viewer::split_rows`）：连续的删除段与紧随的新增段逐行配对、多出的一侧补空；刻意**不做内容相似度匹配**（那需要 LCS，且同一份 diff 在不同实现下可能给出不同配对）。有测试钉住"每一行恰好出现一次（上下文行两次）"这条不变量 —— 它当场抓出了我自己的一处错误（把文件头与 hunk 头当成上下文，于是它们在左右各画了一遍）。<br>真机验证：统一视图（行号 + 色调 + 文件树 + 模式指示 + 操作提示）与双列视图（`---` 在左 / `+++` 在右 / hunk 头单侧 / 分界线）各截图核对。**过程中顺带修掉一个影响所有面板的真缺陷**：面板入场动画原为"下滑 + 淡入"，而**应用空闲时动画停在第 0 帧 → `opacity(0)` → 面板完全看不见**（1100×560 的查看器"打开了"但屏幕上什么都没有）。机制与 §4.64(bi) 同类（平台渲染循环 park 后不再出帧），实测 `cx.notify()` 与 `wake.notify()`（标脏 + refresh）都排不出那一帧。修法：**把可见性从动画里摘出来** —— 恒 `opacity(1)`，动画只做 6px 位移（装饰不得决定可见性）。 | ✅ `PatchProposed{path,diff}` |
...
| D12 | 文件树 / 内置浏览器 / Repo Wiki | ZCode | ⚠️ **文件树已做**（gpui 宿主）：`/files` 命令 + 状态栏开关 → 覆盖式面板列出工作区文件；**点文件查看内容（预览）、引用是预览栏里的独立按钮**。索引在 `neo-platform::file_index`（遵守 `.gitignore`、跳过隐藏、不跟随符号链接、条目/深度/时间三重上限并如实上报 `truncated`）。补了 `neo-protocol::format_file_ref`（此前只有解析没有格式化 —— 含空格路径会静默丢引用）。**整条点击链路已真机验证**（补了 `role(Button)`+`aria_label` 后可语义点击，不必抢焦点）：点 `src/main.rs` → `@src/main.rs`；点 `my file.txt` → `@"my file.txt"`（引号正确）；再点 `plain.rs` → 追加而非覆盖；回车提交后会话日志为 `refs: [my file.txt, plain.rs]` + 两个 `已注入`。扫描准确性另与 `git ls-files` 交叉核对（342 vs 341，唯一差异是未跟踪的新文件）。<br>**内置浏览器（文件预览）已做**：面板两栏（列表 + 预览），点文件看内容 —— 文本逐行带行号、**二进制如实说明不当文本渲染**、读不出来给原因；「加入引用」是预览栏里的独立按钮。读取有界且限定工作区（两道独立的安全闸：入口检查 + canonicalize 比前缀，各自挡一种绕过，见 §4.64(bd)）。<br>**可折叠树已做**：默认全折叠（真实仓库上 342 个文件 → **16 行**：6 个根级目录 + 10 个根级文件）；目录带 ▸/▾ 与**直接子项数**、缩进渲染（层级封顶 8 级防名字被挤出边界）、同级内目录在前；面板有「展开全部 / 全部折叠」。层级推导在共享层 `neo-ui-behavior::tree`（纯逻辑，两个宿主共用）；展开态**按路径记**，重扫后仍保留（见 §4.64(bh)）。<br>**文件监听已做**：磁盘变化 → 树自动更新（事件驱动 + 容量 1 通道合并，无轮询；`.git/` 在事件层跳过、其余靠「扫完再比较」消噪——不重复实现一套忽略规则；启动在后台线程，因实测 FSEvents `watch()` 要 6.2 秒）。真机验证：外部建文件后树自动出现，无用户操作。<br>**Repo Wiki 已做**（用户拍板底线：**禁止打包开发者密钥等私密信息**）：`/wiki` 命令 → 覆盖式面板，左栏目录 + 右栏正文（Markdown 走共享解析器）。收录范围**刻意收窄**：根目录入口文件（README/AGENTS/CONTRIBUTING…）+ `docs/` 下的 Markdown，不是全仓 Markdown 堆积场（收得准比收得多重要）。三重上限（篇数 / 单篇字节 / 总字节）如实上报截断。<br>**底线做成了结构性保证，不是文档提醒**：聚合路径内置敏感信息闸（`neo-platform::secrets`）——**任何内容进 Wiki 之前必须先过检测；命中即整篇排除**，并在界面上报『⚠ N 篇未收录 · 哪一篇 —— 什么类型（第几行）』。三条由此推出的性质：① 闸在聚合路径内部（不在调用方，将来多一个消费者也不会漏）；② 排除是**整篇**而非删行（残缺文档更糟，且作者永远不知道自己泄了东西）；③ **报告不回显密钥原文**（检测器自己不得成为泄露通道）。检测 4 类：PEM/PuTTY 私钥块、已知服务商前缀（sk-/ghp_/AKIA/AIza/sk_live_ 等）、`*_API_KEY=` 形赋值、URL 内嵌凭据；**刻意放过占位符与变量名**（`sk-YOUR_KEY_HERE`、`AKIA…EXAMPLE`、`api_key_env="DEEPSEEK_API_KEY"`）——天天误报的守卫下一步就是被关掉。真机验证：本仓 52 篇文档**零误报**；植入一份含密钥的文档 → 界面出现未收录报告且不泄露密钥原文（见 §4.64(bl)）。<br>egui 侧如实提示『仅 gpui 可用』 | ✅ 完成 |
...
7. ~~**D12**（文件树 / 内置浏览器 / Repo Wiki）~~ ✅ 已完成（含 Repo Wiki）。
