...
| **任务开始选项目**：`projectMode = workspace \| none`；菜单含 **不在项目中工作** / 最近项目 / 打开路径；none 用 `~/.neo/no-project` 会话库（不绑仓库）；**真重启 app-server 换目录** | 只画 chips 不换 workspace |
...
| IA-22 | 任务开始真选项目 / 不在项目中工作 | P1 | ✅ projectMode 真切换 · 专用空项目目录 · 芯片常显 |
| IA-23 | 对话流连续阅读（去卡片割裂） | P1 | ✅ 无助手框 · 中性用户泡 · 工具一行弱化 |
