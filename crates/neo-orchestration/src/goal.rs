...
//!   明确禁止改文件 —— 阶段的意义就是约束行为）。
//! - **推进与判停**：一轮结束 = 引擎 step 一次。审查失败的判据是
//!   **轮内出现 Error 事件或非零退出的工具调用**（内核单点记账，审批拒绝也算）—— 这是确定性的代理判据，
//!   不是模型自评；模型自评审查是更大的一步，诚实边界写在 PROJECT_MEMORY。
//!
//! # 停止条件与确定性
//!
    ...
    fn on_turn_complete(&mut self, usage: (u64, u64), failed: bool) -> Vec<EventMsg> {
        ...
        }
        {
            ...
            if s.stopped.is_none() {
                // 引擎 step 一次：失败（轮内有 Error）→ 审查不过 → 回退重做。
                ...
                if let Some(id) = current_id {
                    if let StepResult::Stopped(reason) = s.engine.step(id, !failed) {
