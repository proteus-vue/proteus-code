...
use dsh_mock::{InMemoryPersistence, ScriptedModelProvider};
use dsh_protocol::{ExecMode, Op, SandboxMode, ToolOutput};
...
impl Tool for ProbeTool {
    ...
    fn call_kind(&self, _a: &Value) -> dsh_core::CallKind { dsh_core::CallKind::Read }
...
fn tool_output_is_capped_and_reports_truncation() {
    let mut k = Kernel::new(
        ...
        Box::new(ScriptedModelProvider::new(vec![
            vec![dsh_mock::tool_call("t1", "probe", serde_json::json!({}))],
            vec![dsh_core::ModelDelta::Text("done".into())],
        ])),
...
fn sandbox_level_truncation_is_propagated() {
    ...
    let mut k = Kernel::new(
        ...
        Box::new(ScriptedModelProvider::new(vec![
            vec![dsh_mock::tool_call("t1", "probe", serde_json::json!({}))],
            vec![dsh_core::ModelDelta::Text("d".into())],
        ])),
        Arc::new(FloodSandbox),
        Box::new(InMemoryPersistence::new()),
        "/tmp",
