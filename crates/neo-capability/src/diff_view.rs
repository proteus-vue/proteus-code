//! Diff 查看器的**纯逻辑**：解析 unified diff、hunk/文件导航、视图模式。
//!
//! # 为什么它在 capability 层，而不是某个宿主里
//!
//! 这个模块原本住在 `neo-host-tui`（TUI 的全屏 diff 查看器）。桌面宿主也要
//! 同一套导航时，有两条路：抄一份，或者上移。抄一份的代价是具体的 ——
//! **同一个 `@@` 头在两个宿主里被解析出不同的 hunk，是最难解释的一类不一致**
//! （TUI 说"下一处改动在这"，GUI 跳去别处）。
//!
//! 更关键的是**它与生产者必须同处一地**：`unified_diff` 就在本 crate 的
//! [`crate::diff`] 里。解析器离生产者越远，格式一改就越容易只有一侧跟上。
//!
//! 所以：解析 + 光标/滚动状态 + 导航语义放这里（可脱离终端单测 ——
//! "按 n 会跳到下一个文件"不该靠截图验证），渲染留在各宿主。
//!
//! # 内存有界
//!
...
impl Viewer {
    ...
    /// # 为什么配对判定在共享层
    ///
    /// "哪一行删对应哪一行增"是**语义**，不是排版：同一个 hunk 在两个宿主里
    /// 若配成不同的对，用户会看到两种不同的对照（而它们代表同一份改动）。
    /// 与 hunk 导航同理 —— 判定放这里，渲染各宿主自己做。
    ///
    /// # 配对规则（简单，但要说清）
    ///
    /// 一段连续的删除行 + 紧随其后的连续新增行**逐行配对**，多出来的一侧
    /// 补空行。这与 `git diff --coalesce` 之后的常见呈现一致：改动通常是
    /// "把这几行换成那几行"，而不是按内容相似度配对。
    ///
    /// **刻意不做内容相似度匹配**：那需要 LCS / 编辑距离，收益是"对照更贴边"，
    /// 代价是同一份 diff 在不同实现下可能给出不同配对（又一个"两处不一致"
    /// 的来源），而 `unified_diff` 本来就已按最小化差异产出。见 ADR 里
    /// "判定唯一"的原则。
    ///
    /// 返回的行与 `Diff::lines` 的**原始下标**一一对应（左、右各一个下标），
    /// 这样调用方要用行号、要按光标定位、要高亮当前行都还有据可查。
    pub fn split_rows(&self) -> Vec<SplitRow> {
        let lines = &self.diff.lines;
        let mut out: Vec<SplitRow> = Vec::new();
        let mut i = 0usize;
        while i < lines.len() {
            match lines[i].kind {
                Kind::Del => {
                ...
                Kind::Add => {
                    // 没有前置删除的新增段（纯插入）：左侧全空
                    while i < lines.len() && lines[i].kind == Kind::Add {
                        out.push(SplitRow { left: None, right: Some(i) });
                        i += 1;
                    }
                }
                Kind::Context => {
                    // 上下文行**同时存在于新旧两侧**，所以两侧指向同一行 ——
                    // 这是唯一一种"一行出现在两处"的情形（渲染时两侧各画一次）。
                    out.push(SplitRow { left: Some(i), right: Some(i) });
                    i += 1;
                }
                Kind::Header => {
                    // 文件头是**两行**：`--- a/x`（旧侧）与 `+++ b/x`（新侧）。
                    // 各自归到对应的一侧 —— 把它们同时放左边会让右栏空一行、
                    // 看起来像"新文件没有头"。
                    let is_new_side = lines[i].text.starts_with("+++");
                    out.push(SplitRow {
                        left: if is_new_side { None } else { Some(i) },
                        right: if is_new_side { Some(i) } else { None },
                    });
                    i += 1;
                }
                Kind::HunkHeader => {
                    // hunk 头不是"内容"，是对后续内容的位置标记，只出现一次。
                    // 一开始把它与上下文归成一类（两侧同行同一行）—— 那是错的：
                    // 会让"每个下标恰好被计入一次"这条不变量失败，也让并排视图
                    // 把 `@@ … @@` 在左右各画一遍（看起来像两边各有一个头）。
                    out.push(SplitRow { left: Some(i), right: None });
                    i += 1;
                }
            }
        }
...
/// 并排视图的一行：左（旧）与右（新）在 `Diff::lines` 里的下标。
///
/// 两侧都为 `None` 是不可能的（不产出这样的行）—— 但字段用 `Option` 是因为
/// 「这一侧没有内容」正是配对补齐的情形，而它必须能被渲染出来（画空的半行）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct SplitRow {
...
impl SplitRow {
    /// 这一行是否与某侧的内容有关（用于"当前行高亮"这类判断）。
    pub fn contains(&self, idx: usize) -> bool {
        self.left == Some(idx) || self.right == Some(idx)
    }
}
...
+q";
    ...
    /// 基本的"删除段 + 新增段"逐行配对，且多余的一侧补空。
    #[test]
    fn split_rows_pairs_deletions_with_additions() {
        let d = parse(SAMPLE);
        let v = Viewer::new(d);
        let rows = v.split_rows();

        // 只看**第一处改动**（样例里的第一个 hunk）：
        //   -    old_call();      → 左：删除
        //   +    new_call();        配对成一行（两侧都有）
        //   +    extra();           多出来的一行 → 左侧补空
        // 判据：取第一行删除之前的行数，之后按顺序看三行。
        let del_idx = v
            .diff
            .lines
            .iter()
            .position(|l| l.kind == Kind::Del)
            .expect("样例有删除行");
        // 找到含该删除行的那一"对"
        let pair_at = rows
            .iter()
            .position(|r| r.contains(del_idx))
            .expect("删除行必须出现在并排视图里");

        let r0 = rows[pair_at];
        let r1 = rows[pair_at + 1];
        assert!(
            r0.left.is_some() && r0.right.is_some(),
            "第一行应是「删↔增」成对：{r0:?}"
        );
        assert_eq!(r0.left, Some(del_idx), "左侧应指向那行删除");
        assert!(
            r1.left.is_none() && r1.right.is_some(),
            "多出来的新增行应左侧补空：{r1:?}"
        );
        // 且这两行的右侧内容确实就是那两行新增（顺序不能颠倒）
        let add_texts: Vec<&str> = [r0.right, r1.right]
            .iter()
            .flatten()
            .map(|i| v.diff.lines[*i].text.as_str())
            .collect();
        assert_eq!(add_texts, vec!["    new_call();", "    extra();"]);
    }

    /// 上下文行与头行**两侧同行同一行**（不是两行）。
    #[test]
    fn split_rows_keeps_context_on_both_sides() {
        let d = parse(SAMPLE);
        let v = Viewer::new(d);
        let rows = v.split_rows();
        let ctx_row = rows
            .iter()
            .find(|r| r.left.is_some_and(|i| v.diff.lines[i].kind == Kind::Context))
            .expect("样例里有上下文行");
        assert_eq!(ctx_row.left, ctx_row.right, "上下文行两侧应是同一行");
    }

    /// 纯插入（只有 + 没有 -）：左侧全空，右侧逐行。
    #[test]
    fn split_rows_handles_pure_insertions() {
        let d = parse("--- a/f\n+++ b/f\n@@ -1,1 +1,3 @@\n keep\n+new1\n+new2");
        let v = Viewer::new(d);
        let rows = v.split_rows();
        let adds: Vec<&SplitRow> = rows
            .iter()
            .filter(|r| r.right.is_some_and(|i| v.diff.lines[i].kind == Kind::Add))
            .collect();
        assert_eq!(adds.len(), 2);
        assert!(adds.iter().all(|r| r.left.is_none()), "纯插入的左侧应为空: {adds:?}");
    }

    /// **配对不丢行**：每一行都必须恰好出现在某一行的某一侧 —— 否则并排视图
    /// 会静默漏掉内容（比顺序错更难发现）。
    #[test]
    fn split_rows_accounts_for_every_line_exactly_once() {
        let d = parse(SAMPLE);
        let v = Viewer::new(d);
        let rows = v.split_rows();
        let mut seen = vec![0usize; v.diff.lines.len()];
        for r in &rows {
        ...
        for (i, count) in seen.iter().enumerate() {
            assert!(*count >= 1, "第 {i} 行没出现在并排视图里：{:?}", v.diff.lines[i]);
        }
        // 出现次数：上下文行两侧各一次（=2）；其余行恰好一次。
        // ⚠️ 这条守的是"并排视图不漏行、也不重复画"—— 头行也曾被归成两侧
        // 同行同一行（=2），那是实现错误，正是这条断言抓出来的。
        for (i, count) in seen.iter().enumerate() {
            let expected = if v.diff.lines[i].kind == Kind::Context { 2 } else { 1 };
            assert_eq!(
                *count, expected,
                "第 {i} 行出现 {count} 次（应为 {expected}）：{:?}",
                v.diff.lines[i]
            );
        }
    }

    /// 空 diff 不 panic，也没有配出行。
    #[test]
    fn split_rows_on_empty_diff_is_empty() {
        let v = Viewer::new(Diff::default());
        assert!(v.split_rows().is_empty());
    }
    ...
    fn parses_diffs_produced_by_the_capability_layer() {
        ...
        let far = 10 + 2 * crate::diff::CONTEXT;
        let n = far + crate::diff::CONTEXT + 10;
