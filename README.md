...
| **桌面 GPUI 宿主**（**默认桌面 UI**，2026-09 转正） | ✅ **已转正**：`neo desktop` 默认打开 GPUI 窗口。**D1–D12 parity 已全部覆盖**（含 D5 的全屏 diff 查看器：`/diff` 或审批时按 `d`，统一/双列切换 + 跳转上/下一处改动，键位与 TUI 逐键一致），并在三处领先旧实现（思考轨迹搜索、工具调用分组、只在底部才跟随的自动跟随）。真机验证：中文、D2–D12 全部渲染正确，打字→回车提交→审批→落盘全流程通过，1x/2x 两档 DPI 无错位，IME 有自动化回归用例。它一行 gpui 依赖都不直接声明 —— 全部经 UI 栈取用（单一 pin，门禁强制）。<br>**D12 含 Repo Wiki**（`/wiki`）：把仓库文档（根目录入口文件 + `docs/` 下的 Markdown）聚合成目录 + 正文两栏。**聚合前过一道敏感信息闸**（`neo-platform::secrets`）：命中疑似密钥/私钥/凭据的文档**整篇排除**并上报（哪一篇、什么类型、第几行），**不回显密钥原文** —— 即「绝不打包开发者私密信息」是可判定的守卫，而不是靠自觉。**egui 未删**：`--egui` 保留为回退通道，待稳定后删（阶段 3 剩余项） |
...
│   ├── neo-platform/          L1 平台（进程加固 / fs notify / git / 文件索引 / Repo Wiki + 密钥检测闸）
