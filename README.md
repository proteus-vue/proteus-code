...
| `neo tui` 终端宿主（MiMo Code 风格：星场首页 + 居中构图 + 带边框输入框 + 信任门） | ✅ **已实现**，pty 验证（信任/首页/对话三态；256/truecolor + NO_COLOR 降级） |
...
cargo test --workspace     # 134 个测试（内核 conformance / 内存有界性 / SPI / 宿主 / TUI / Web）
